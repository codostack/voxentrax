    ...
    <div className="flex items-end justify-center gap-px" style={{ height: 26 }}>
      {delays.map((d, i) => (
        <div key={i} className="rounded-sm" style={{
          ...
          animation: `barAnim 0.6s ease-in-out infinite ${d}s`,
        }} />
      ))}
    </div>
  );
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center py-7 px-6 bg-white">
        <p className="text-[25px] font-medium tracking-widest uppercase text-orange-400 mb-6 text-center">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-center w-full max-w-5xl gap-6">
          ...
          {/* CENTER CANVAS */}
          <div className="flex-1 relative w-full" style={{ height: 360, minWidth: 260, maxWidth: 420 }}>
          ...
          <div className="flex flex-col items-center gap-2">
            <div className="bg-white rounded-2xl relative overflow-hidden flex flex-col justify-between"
              style={{ border: "1px solid #dd6712", padding: "14px 12px", width: 180, height: 306, boxShadow: "0 2px 12px rgba(0,0,0,.06)" }}>
              {[60, 84, 108].map((size, i) => (
                <div key={i} className="absolute rounded-full" style={{ width: size, height: size, border: "1.5px solid #f97316", top: "50%", left: "50%", opacity: 0, animation: `pulseRing 1.8s ease-out infinite ${i * 0.5}s`, pointerEvents: "none", zIndex: 0 }} />
              ))}
              <div className="text-[10px] text-slate-400 text-center mb-2 tracking-wide uppercase relative z-10">Receiver</div>
              <div className="flex flex-col items-center justify-center flex-1 gap-2 relative z-10">
                <div className="flex items-center justify-center rounded-full text-[13px] font-medium" style={{ width: 36, height: 36, background: "#FFF4E6", color: "#c2410c" }}>B</div>
                <div className="text-[10px] text-slate-400" style={{ animation: "blinkDot 1.2s ease-in-out infinite" }}>Incoming…</div>
              </div>
              <div className="flex justify-center gap-2.5 relative z-10 mb-3">
                <div className="flex items-center justify-center rounded-full" style={{ width: 28, height: 28, background: "#FFF4E6" }}>
                ...
                <div className="flex items-center justify-center rounded-full" style={{ width: 28, height: 28, background: "#1D9E75" }}>
        ...
        {/* STATUS BAR */}
        <div className="mt-5 flex flex-wrap items-center justify-center gap-3 px-5 py-2 rounded-full bg-slate-50" style={{ border: "1px solid #ec6b25" }}>
...
function LiveCallGraph() {
  ...
  useEffect(() => {
    ...
    function drawGraph() {
      ...
      ctx.setLineDash([3, 6]); ctx.strokeStyle = "rgba(148,163,184,0.10)"; ctx.lineWidth = 0.5;
      ...
      ctx.strokeStyle = "#64748b"; ctx.lineWidth = 1.5; ctx.lineJoin = "round"; ctx.stroke();

      const last = pts[pts.length - 1];
      const pulse = Math.sin(phaseRef.current * 3);
      ctx.beginPath(); ctx.arc(last.x, last.y, 9 + pulse * 2.5, 0, Math.PI * 2); ctx.fillStyle = "rgba(24,95,165,0.10)"; ctx.fill();
      ctx.beginPath(); ctx.arc(last.x, last.y, 5.5 + pulse * 1.2, 0, Math.PI * 2); ctx.fillStyle = "rgba(24,95,165,0.22)"; ctx.fill();
      ctx.beginPath(); ctx.arc(last.x, last.y, 3.5, 0, Math.PI * 2); ctx.fillStyle = "#64748b"; ctx.fill();
      ctx.beginPath(); ctx.arc(last.x, last.y, 1.5, 0, Math.PI * 2); ctx.fillStyle = "#fff"; ctx.fill();

      ctx.setLineDash([2, 3]); ctx.strokeStyle = "rgba(24,95,165,0.20)"; ctx.lineWidth = 0.8;
      ctx.beginPath(); ctx.moveTo(last.x, last.y + 4); ctx.lineTo(last.x, h - pad.b); ctx.stroke();
      ctx.setLineDash([]);
    }
    ...
    function tick() {
      ...
      const wave = Math.sin(phaseRef.current) * 40 + Math.sin(phaseRef.current * 0.5) * 25;
      const noise = Math.random() * 20 - 10;
      let next = current + wave * 0.1 + noise;
      next = Math.max(8500, Math.min(15000, next));
      countRef.current = Math.round(next);
      dataRef.current = [...dataRef.current.slice(1), countRef.current];
    ...
    drawGraph();
    timerRef.current = setTimeout(tick, 750);
    return () => { clearTimeout(timerRef.current); window.removeEventListener("resize", resize); };
  }, []);
      ...
      <div className="flex items-start justify-between px-4 pt-3 pb-2">
        <div>
          ...
          <div className="flex items-end gap-2">
            ...
            <span className="text-[11px] font-medium mb-0.5 px-1.5 py-0.5 rounded-md"
              style={{ background: trendUp ? "#EAF3DE" : "#FCEBEB", color: trendUp ? "#3B6D11" : "#A32D2D" }}>
              {trendUp ? "▲" : "▼"} {trend}
            </span>
          </div>
        </div>
...
function SuccessScreen({ name, email }) {
  return (
    <div className="min-h-screen bg-white flex items-center justify-center font-sans px-4">
      <div className="bg-white border border-slate-200 p-8 sm:p-12 rounded-2xl text-center w-full max-w-sm">
  ...
  return (
    <div className="bg-white min-h-screen font-sans">
      <style>{`
        ...
        /* ── VoIP banner: visible only on screens >= 1029px ── */
        .voip-banner {
          display: none;
        }
        ...
        /* ── Page grid: single column below 1029px, two columns above ── */
        .page-grid {
          display: grid;
      ...
      {/* ── VoIP Animation Banner — hidden below 1029px ── */}
      <div className="voip-banner max-w-6xl mx-auto mt-10 rounded-2xl overflow-hidden border border-slate-100">
        <VoipAnimation />
      </div>
