...
const RateTable = ({ selectionMode = false }) => {
  ...
  // ✅ 🔥 IMPORTANT: re-translate after pagination / filter / language change
  useEffect(() => {
    if (!language || language === "en") return;

    const timer = setTimeout(() => {
      if (window.retranslatePage) {
  ...
  return (
    ...
    <div style={{ padding: "40px" }}>
      ...
      {/* Table */}
      <div key={currentPage} style={{ border: "1px solid #e5e7eb", borderRadius: 8, overflowX: "auto" }}>
          ...
          <tbody>
            {loading ? (
              ...
              paginatedData.map((rate, index) => (
                <tr key={rate._id || index} className={index % 2 === 0 ? "bg-white" : "bg-gray-100"}>
